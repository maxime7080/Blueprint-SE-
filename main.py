...
selected_lang = "en"  # valeur par défaut, sera mise à jour dynamiquement
...
def extract_pcu_values():
    ...
    for fname in os.listdir(cubeblock_path):
        if fname.endswith(".sbc"):
            try:
                tree = ET.parse(os.path.join(cubeblock_path, fname))
                root = tree.getroot()
                for defn in root.findall(".//Definition"):
                    subtype = defn.findtext("Id/SubtypeId")
...
def start_interface(lang):
    ...
    pcu_label = tk.Label(root, text="", bg="#2e2e2e", fg="white", font=("Arial", 10, "italic"))
    pcu_label.pack(pady=(0, 10))

    tk.Button(btn_frame, text="Importer blueprint", command=lambda: browse_file(), bg="#444", fg="white").pack(side="left", padx=10)
    def switch_language():
        global selected_lang
        selected_lang = "en" if selected_lang == "fr" else "fr"
        root.destroy()
        start_interface(selected_lang)

    
    lang_tip = {"fr": "Cliquez pour changer la langue", "en": "Click to switch language"}
    tk.Label(btn_frame, text=lang_tip[selected_lang], bg="#2e2e2e", fg="white").pack(side="left", padx=5)
    tk.Button(btn_frame, text=f"Langue : {'Français' if selected_lang == 'fr' else 'English'}",
              command=switch_language, bg="#444", fg="white").pack(side="left", padx=10)
    ...
    # Cadre pour l'image au centre de l'écran
    image_frame = tk.Frame(root, bg="#2e2e2e")
    image_frame.place(relx=0.85, rely=0.09, anchor="center")

    # Créer le canvas une seule fois
    f_img = tk.Canvas(image_frame, bg="#C2C2C2", height=81, width=144)
    f_img.pack()
    ...
    def create_listbox(tab, path):
        ...
        def on_select(event):
            ...
            image_path = os.path.join(path, name, "thumb.png")
            if os.path.exists(file_path):
                ...
                img_redim = image.resize((144, 81))
                photo = ImageTk.PhotoImage(img_redim)

                # Conserver une référence à l'image
                f_img.image = photo

                # Mettre à jour l'image dans le canvas
                f_img.delete("all")  # Effacer l'image précédente
                f_img.create_image(73, 40, image=photo)
    ...
    comp_tree = create_tree(tree_frame, "Component-Composants")
    ingot_tree = create_tree(tree_frame, "Ingot-Lingots")
    block_tree = create_tree(tree_frame, "Blocks-Blocs")   

    

    def show_result(components, blocks, ingots):
        total_pcu = sum(pcu_map.get(b, 0) for b in blocks)
        pcu_label.config(text=f"Total PCU: {total_pcu}")
